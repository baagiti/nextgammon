...
import { Swords, Lock, CheckCircle2, Trophy, AlertTriangle, Bot } from 'lucide-react';
...
export const RunMapModal: React.FC<RunMapModalProps> = ({ run, onEnterMatch }) => {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto grain bg-panel border-2 border-line-strong rounded-2xl p-4 sm:p-6 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.7)] text-text">
      ...
      {/* Captured cards warning */}
      {run.capturedCardIds.length > 0 && (
        <div className="relative z-10 mb-5 bg-danger/10 border border-danger/50 rounded-xl p-3 flex items-start gap-3">
          ...
          <div className="text-sm">
            ...
            <span className="text-text-muted"> — clear this stage to recover {run.capturedCardIds.length > 1 ? 'them' : 'it'}.</span>
          </div>
        </div>
      )}
      ...
      {currentStage && (
        ...
        >
          ...
          <button
            ...
            className="shrink-0 py-3 px-6 rounded-xl bg-player text-ink font-display font-black text-sm uppercase tracking-wider shadow-[0_0_25px_var(--player)]/60 hover:scale-105 hover:brightness-110 transition-all flex items-center justify-center gap-2"
          >
            BATTLE OPPONENT
